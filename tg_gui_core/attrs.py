...
class WidgetAttr(Generic[_Attr]):
    ...
    # make sure the return type of `__new__` is `Any`, pyright uses this for dataclass attr inference
    #    ```
    #    @widget
    #    class Foo(View):
    ...
    def __widattr_init__(
        ...
        *,
        ...
        if default is not Missing:
            assert (
                default_factory is Missing
            ), "cannot set both default and default_factory"
...
def _widget(cls: Type[_W]) -> Type[_W]:
    ...
    # this is used to validate that the @widget decorator is called on widget classes and
    assert (
